...
def generate_launch_description():
    ...
    v1_base_to_footprint_ekf = Node(
        package="robot_localization",
        executable="ekf_node",
        name="v1_base_to_footprint_ekf",
        output="screen",
        parameters=[
            ...
            os.path.join(
                ...
                "config",
                "ekf",
                "v1_base_to_footprint.yaml",
            ),
        ],
        remappings=[("odometry/filtered", "odom/local")],
    )

    v1_footprint_to_odom_ekf = Node(
        package="robot_localization",
        executable="ekf_node",
        name="v1_footprint_to_odom_ekf",
        output="screen",
        parameters=[
            {"base_link_frame": LaunchConfiguration("base_link_frame")},
            {"use_sim_time": LaunchConfiguration("use_sim_time")},
